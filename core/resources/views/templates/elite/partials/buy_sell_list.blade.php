...
@endphp
<section class="banner-section" style="background-color: #D2DDDD; position: relative; overflow: hidden;">
        ...
        <div class="row align-items-center">
            <div class="col-lg-7">
                <div class="banner-content">
                    <h5 class="banner-content__subtitle" style="color: #1D5550;font-size: xx-large;font-weight: bold;">{{ __(@$content->data_values->heading) }}</h5>
                   <h5 class="banner-content__title" s-break="3" s-color="base-two" s-length="2" style="color: #1D5550;font-size:small;">{{ __(@$content->data_values->subheading) }}</h5>
                </div>
            </div>
            <div class="col-lg-5">
                    ...
                    <div class="tag-list">
                        @foreach ($cryptos as $crypto)
                            <span class="tag-list__item @if (@$request->crypto == $crypto->code ) active @elseif(!@$request->crypto && $loop->first) active @endif crypto--symbol cursor-pointer"
                                data-id="{{ $crypto->id }}" data-code="{{ $crypto->code }}" data-name="{{ $crypto->name }}" style="background-color:#D2DDDD; color:#1D5550; border: 1px solid #1D5550">
                                {{ __($crypto->code) }}
                            </span>
                        @endforeach
                    </div>
                ...
                <form action="#" method="get" class="ptable-filter row buy--sell-filter">
                    ...
                    <div class="col-xl-2 col-sm-6 filter--btn-wrapper">
                        <div class="form-group d-flex flex-column gap-2">
                            <button type="submit" class="btn btn--base filter--btn" disabled>
                                <i class="las la-filter"></i> @lang('Filter')
                            </button>
                            <button type="button" class="btn btn--light reset--btn d-none">
...
<style>
    ...
    .banner-animation .stars, .banner-animation .bubbles {
        position: absolute;
        width: 100%;
        height: 100%;
        background: url('path-to-your-stars-or-bubbles-image.png') repeat;
        animation: move 20s linear infinite;
    }
    .banner-animation .bubbles {
        background: url('path-to-your-bubbles-image.png') repeat;
    }
    @keyframes move {
